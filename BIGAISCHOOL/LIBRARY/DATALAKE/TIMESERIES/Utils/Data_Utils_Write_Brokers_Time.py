...
def Write_Brokers_Opening_Times(Chart):
    ...
    column_names = ["Broker", "Period", "OpenTime"]
    df = pd.DataFrame(columns=column_names)
    df = df.append({"Broker": Chart.Broker, "Period": Chart.Period, "OpenTime": Chart.OpenTimes[-1]}, ignore_index=True)
...
def Write_Brokers_Closing_Times(Chart):
    ...
    data_path_last = fspath(data_path)
    column_names = ["Broker", "Period", "CloseTime"]
    df = pd.DataFrame(columns=column_names)
    df = df.append({"Broker": Chart.Broker, "Period": Chart.Period, "CloseTime": Chart.ClosingTimes[-1]}, ignore_index=True)
    if os.path.isfile(data_path_last):
        df_from_file = pd.read_csv(data_path_last)
        frames = [df_from_file, df]
        result = pd.concat(frames)
